 ...
 * @since   1.0.0
 */
class GigsModule extends AbstractModule {
	/**
	...
	public function register_hooks() {
		$this->plugin->register_hooks( new PostType\Gig( $this ) );
		$this->plugin->register_hooks( new PostType\Venue() );
		...
		if ( is_admin() ) {
			...
			$this->plugin->register_hooks( new Screen\ManageGigs() );
			$this->plugin->register_hooks( new Screen\EditGig() );
			$this->plugin->register_hooks( new Screen\ManageVenues() );
			$this->plugin->register_hooks( new Screen\EditVenue() );
		}
	}
	...
	public function get_google_maps_api_key() {
		$option_name = Provider\Setting\GoogleMaps::API_KEY_OPTION_NAME;
	...
	public function register_rest_routes() {
		$controller = new REST\GigsController( 'bandstand/v1', 'gigs', 'bandstand_gig' );
		$controller->register_routes();

		$controller = new REST\VenuesController( 'bandstand/v1', 'venues', 'bandstand_venue' );
		$controller->register_routes();
	}
