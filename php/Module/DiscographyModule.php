 ...
 * @since   1.0.0
 */
class DiscographyModule extends AbstractModule {
	/**
	...
	public function register_hooks() {
		$this->plugin->register_hooks( new Taxonomy\Genre( $this ) );
		$this->plugin->register_hooks( new Taxonomy\RecordType( $this ) );
		$this->plugin->register_hooks( new PostType\Playlist( $this ) );
		$this->plugin->register_hooks( new PostType\Record( $this ) );
		$this->plugin->register_hooks( new PostType\Track( $this ) );
		$this->plugin->register_hooks( new AJAX\Discography() );
		...
		if ( is_admin() ) {
			$this->plugin->register_hooks( new Screen\ManageRecords() );
			$this->plugin->register_hooks( new Screen\ManageTracks() );
			$this->plugin->register_hooks( new Screen\EditRecord() );
			$this->plugin->register_hooks( new Screen\EditTrack() );
			$this->plugin->register_hooks( new Screen\EditRecordArchive() );
		}
	}
	...
	public function register_rest_routes() {
		$controller = new REST\RecordsController( 'bandstand/v1', 'records', 'bandstand_record' );
		$controller->register_routes();

		$controller = new REST\TracksController( 'bandstand/v1', 'tracks', 'bandstand_track' );
		$controller->register_routes();
	}
